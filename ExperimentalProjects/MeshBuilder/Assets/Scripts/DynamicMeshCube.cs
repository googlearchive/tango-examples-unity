...
public class DynamicMeshCube : MonoBehaviour {
     ...
     * @param voxelResolution number of voxel divisions in this cube.
     */
    ...
    public int ComputeVoxelHashKey(int x, int y, int z) {
        return x+m_voxelMarginCount + (y+m_voxelMarginCount)*m_voxelGridDimension + (z+m_voxelMarginCount)*m_voxelGridDimension*m_voxelGridDimension;
    }
    ...
    public int ComputeVoxelHashKey(Vector3 p) {
        return ((int)Mathf.Floor(p.x))+m_voxelMarginCount + (((int)Mathf.Floor(p.y))+m_voxelMarginCount)*m_voxelGridDimension + (((int)Mathf.Floor(p.z))+m_voxelMarginCount)*m_voxelGridDimension*m_voxelGridDimension;
    }
     ...
     * @return list of populated voxels that are intersected by the ray
     */
     ...
     * @param x x query index
     * @param y y query index
     * @param z z query index
     * @return the voxel if it exists, null if it does not exist
     */
    ...
    /**
     * Query if a voxel exists at the query location.  If ti does not exits, create it.
     * @param x x query index
     * @param y y query index
     * @param z z query index
     * @return the voxel
     ...
     * @param initialValue initialValue of the voxel
     * @param initialWeight initialWeight of the voxel
     * @return the created voxel
     */
    Voxel InitializeVoxel(int xID, int yID, int zID, float voxelSize, float initialValue, float initialWeight) {
        Voxel v = new Voxel();
        v.size = voxelSize;
     ...
     * @param v the Voxel to be adjusted
     * @param p the point inserted
     * @param obs observation direction of the point
     * @param weight weight of the observation 
     * @return final value of the adjusted voxel
     */
    float AdjustWeight(Voxel v, Vector3 p, Vector3 obs, float weight) {
     ...
     * @param obs observation direction of this point
     * @param weight of the observation
     * @param index output position index of the voxel
     * @return the final value of the adjusted voxel
     */
    public float InsertPoint(Vector3 p, Vector3 obs, float weight, ref int[] index) {

        if (m_isRegenerating)
            return -1;
        index[0] = (int)Math.Floor((p.x-transform.position.x)/m_voxelSize);
        index[1] = (int)Math.Floor((p.y-transform.position.y)/m_voxelSize);
        index[2] = (int)Math.Floor((p.z-transform.position.z)/m_voxelSize);
    ...
    public float InsertPoint(Vector3 queryPoint, Vector3 p, Vector3 obs, float weight) {
        ...
        int x = (int)Math.Floor ((queryPoint.x - transform.position.x) / m_voxelSize);
        int y = (int)Math.Floor ((queryPoint.y - transform.position.y) / m_voxelSize);
        int z = (int)Math.Floor ((queryPoint.z - transform.position.z) / m_voxelSize);
    ...
    /**
     * Walks through the voxel space prior to meshing to ensure all populated voxel have neighbors that are below the isosurface.
     * Having populated voxels surrounded by neighbors below the isosurface is required to create valid meshes.
     */
    void PrepareVoxels() {
     ...
     * @param ia first triangle vertex index
     * @param ib second triangle vertex index
     * @param ix third triangle vertex index
     */
     ...
     * @param v the voxel to mesh
     * @return number of created triangles
     */
    int Polygonize(Voxel v) {
        if (v == null)
            return 0;
        ...
        int createdTriangles = Polygonizer.Process(m_meshingIsolevel,
                            ...
                            v6.anchor,
                            v7.anchor,
                            ref m_vertices,ref m_triangles);
